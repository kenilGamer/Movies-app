...
function Topnev({lef}) {
  ...
  return (
    <div className='w-full h-[10vh]  relative flex items-center justify-center'>
       ...
       <input onChange={((e)=>setquery(e.target.value))} value={query} className='min-w-[40%] py-4 px-16 mt-5 outline-none text-zinc-200  font2 bg-[#20164d] rounded-full text-xl mx-1 flex items-center justify-center shadow-md shadow-[#000033] ' type="search" name="search" placeholder='search anything'/>
        <RiSearchEyeLine className={`text-3xl mt-5 text-zinc-400 -ml-16  `}/>
       </div>
       {/* ${lef && true ? "left-[34vw]" : " left-[52vw] "}/ */}
     {query.length > 0 &&  
      <div className='w-[40%] rounded-xl z-20 max-h-52 absolute top-[120%] py-3 px-1 bg-[#111] gap-2 flex flex-col overflow-auto'>
        {searchs.map((i,x)=>(
          ...
          <span>{i.titlel || i.original_name || i.original_title}</span>
          <img src={`https://image.tmdb.org/t/p/original/${i.poster_path || i.backdrop_path || i.profile_path}`} className='w-[10vw] rounded-xl overflow-hidden' alt={``} />
          </Link>
        ))}
       </div>}
    </div>
