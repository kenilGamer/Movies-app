...
import { FaArrowLeft, FaStar, FaPlay, FaBookmark, FaHeart, FaExternalLinkAlt, FaGlobe, FaImdb, FaClock, FaCalendarAlt, FaTv } from "react-icons/fa";
...
const addToHistory = (newItem) => {
  // Retrieve the current history from localStorage (if any)
  const storedHistory = localStorage.getItem("history");

  // If history doesn't exist yet, create an empty array
  ...
  if (!isAlreadyInHistory) {
    // If the item is not in history, add it to the array
    historyArray.push(newItem);

    // Optionally, limit the size of the history array (e.g., keep the last 10 items)
    if (historyArray.length > 10) {
      historyArray = historyArray.slice(-10); // Keep only the last 10 items
    }

    // Store the updated history back to localStorage as a JSON string
          ...
          <div className="mt-4 sm:mt-6 mb-6 flex flex-wrap items-center gap-3 sm:gap-4">
            ...
            {info.detail.first_air_date && (
              <div className="flex items-center gap-2 bg-black/40 backdrop-blur-md px-3 sm:px-4 py-2 rounded-xl border border-white/10">
                ...
                <span className="text-sm sm:text-base">{info.detail.first_air_date}</span>
              </div>
            )}
            ...
            {info.detail.number_of_episodes && (
              <div className="flex items-center gap-2 bg-black/40 backdrop-blur-md px-3 sm:px-4 py-2 rounded-xl border border-white/10">
                <FaTv className="text-indigo-400 text-sm sm:text-base" />
                <span className="text-sm sm:text-base">
                  {info.detail.number_of_seasons} Season{info.detail.number_of_seasons !== 1 ? 's' : ''} • {info.detail.number_of_episodes} Episodes
                </span>
              </div>
            )}
        ...
        <div className="relative z-10 w-full px-4 sm:px-6 mt-8 sm:mt-12 mb-8 sm:mb-12">
          ...
          <div className="flex gap-4 sm:gap-6 overflow-x-auto pb-4 scrollbar-hide scroll-smooth">
            {info.detail.seasons.map((s, i) => (
              <div
                key={i}
                className="group flex-shrink-0 w-[40vw] sm:w-[25vw] md:w-[20vw] lg:w-[18vw] hover:scale-105 duration-300 cursor-pointer"
              >
                <div className="relative overflow-hidden rounded-xl sm:rounded-2xl bg-gradient-to-br from-zinc-900/50 to-zinc-800/30 backdrop-blur-sm border border-zinc-800/50 shadow-xl shadow-black/40 group-hover:shadow-2xl group-hover:shadow-indigo-500/30 group-hover:border-indigo-500/50 transition-all duration-500">
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-4 transform translate-y-full group-hover:translate-y-0 transition-transform duration-500 bg-gradient-to-t from-black/95 to-transparent">
                    <h3 className="text-base sm:text-lg font-bold text-white mb-1 line-clamp-2">{s.name}</h3>
                    <p className="text-sm text-zinc-300">
                      {s.episode_count} episode{s.episode_count !== 1 ? 's' : ''}
                    </p>
                  </div>
                </div>
                <h3 className="text-base sm:text-lg text-white mt-3 font-semibold line-clamp-2">{s.name}</h3>
